...
from fishing_core.utils import check_boat_tier
...
class BattleCog(commands.Cog):
    ...
    @app_commands.command(name="통", description="배틀에 출전할 수 있는 통(배틀 전용 저장소)을 확인합니다.")
    async def 통(self, interaction: discord.Interaction):
        async with db.conn.execute("SELECT item_name, amount FROM bucket WHERE user_id=? AND amount > 0", (interaction.user.id,)) as cursor:
            items = await cursor.fetchall()
